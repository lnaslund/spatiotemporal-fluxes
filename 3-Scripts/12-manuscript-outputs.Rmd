...
ysi_summary %>% filter(sampling == "Blue Herron 13-14 Sept")
...
# n
suncloud %>% filter(is.na(time) == FALSE & is.na(wolffia)==FALSE & is.na(sampling)==FALSE & is.na(flux_co2_g_hr)==FALSE) %>% nrow()
```
...
```{r}
space_sims_combos %>% mutate(perc = 100* (co2_eq_diff/calc_co2_eq) ) %>% summarize(min = min(co2_eq_diff), max = max(co2_eq_diff), min_perc = min(perc), max_perc = max(perc))
```
...
time_combos %>% filter(ord1 == ord2) %>% mutate(perc = 100*(co2_eq_diff/calc_co2_eq)) %>% summarize(min = min(co2_eq_diff), max = max(co2_eq_diff), min_perc = min(perc), max_perc = max(perc))
...
tiff(filename = "../4-Figures/co2-eq-summary.tiff", width = 7.5, height = 5, units = "in", res = 1200)
all_fluxes_pathways %>% 
  ...
  scale_fill_manual(values = c("#BFDBC0","#94A7C5", "#3D5170"), labels = c(expression(CO[2]~diffusion), expression(CH[4]~diffusion), expression(CH[4]~ebullition)))+
  ylab(expression(CO[2-eq]~ flux~(g~m^-2~d^-1)))+
  theme_bw()+
  theme(legend.text.align = 0, axis.title.x = element_blank(), legend.title = element_blank(), axis.text.x = element_text(angle=45, hjust=1), axis.text = element_text(size = 12, color = "black"), axis.title = element_text(size = 14, color = "black"), legend.text = element_text(size = 12))
dev.off()
```
...
tiff(filename = "../4-Figures/diel-patterns_co2_eq.tiff", width = 8, height = 15, units = "in", res = 1200)
plot_grid(co2_diel + geom_hline(yintercept = 0, color = "red", linetype = "dashed"), ch4_diel+ geom_hline(yintercept = 0, color = "red", linetype = "dashed"), eb_diel+ geom_hline(yintercept = 0, color = "red", linetype = "dashed"), co2_eq_diel+ geom_hline(yintercept = 0, color = "red", linetype = "dashed"), ncol = 1, labels = "AUTO", label_size = 18, vjust = 1.3, align = "v")
dev.off()

png(filename = "../4-Figures/diel-patterns_co2_eq_free.png", width = 9, height = 15, units = "in", res = 600)
plot_grid(co2_diel_free + geom_hline(yintercept = 0, color = "red", linetype = "dashed"), ch4_diel_free + geom_hline(yintercept = 0, color = "red", linetype = "dashed"), eb_diel_free + geom_hline(yintercept = 0, color = "red", linetype = "dashed"), co2_eq_diel_free + geom_hline(yintercept = 0, color = "red", linetype = "dashed"), ncol = 1, labels = "AUTO", label_size = 18, vjust = 1.3, align = "v")
dev.off()
```

Figure 3: Iterative site number consequences
```{r fig 3}
tiff(filename = "../4-Figures/interate_space.tiff", width = 8, height = 12, units = "in", res = 1200)
...
```{r fig 4}
tiff(filename = "../4-Figures/diffusion_heatmaps.tiff", width = 17.5, height = 11.5, units = "in", res = 1200)
plot_grid(co2_diff_hm, ch4_diff_hm, ch4_eb_hm, co2_eq_hm, labels = "AUTO", label_size = 20)
dev.off()
```
...
png("../4-Figures/point_maps_ch4.png", width = 12, height = 15, units = "in", res = 300)
tmap_arrange(pick_ch4_13, catfish_ch4_06, catfish_ch4_18, deans_ch4_16,  deans_ch4_30, sister_ch4_22, ncol = 2, outer.margins = 0.05)
dev.off()

png("../4-Figures/point_maps_eb.png", width = 12, height = 15, units = "in", res = 300)
tmap_arrange(pick_eb_13, catfish_eb_06, catfish_eb_18, deans_eb_16,  deans_eb_30, sister_eb_22, ncol = 2, outer.margins = 0.05)
dev.off()
```
...
```{r}
table_s2 <- all_fluxes_pathways  %>% select(site, date, pathway, gas, co2_eq_conv, flux_mean_g_m2_day:flux_97.5_g_m2_day)  

table_s2

table_s2 %>% mutate(mean_co2_eq = flux_mean_g_m2_day * co2_eq_conv, lower_co2_eq = flux_2.5_g_m2_day * co2_eq_conv, upper_co2_eq = flux_97.5_g_m2_day * co2_eq_conv) %>% group_by(site, date) %>% summarize(mean_co2_eq_sum = sum(mean_co2_eq), lower_co2_eq_sum = sum(lower_co2_eq), upper_co2_eq_sum = sum(upper_co2_eq))
```
...
NEON gas sampling times
```{r neon mode, include = F}
# library(neonUtilities)
# 
# token <- "eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzI1NiJ9..4QnGrN0icYdVuyalW2aIUDBF9lsAewbk40M34c6DxXgMzV9xSzlMIN82ZncrC49Bm-34hWUi1S-KtWfkCnPXTA"
# 
...
ggplot(neon %>% mutate(dt = as_datetime(time_num)), aes(dt))+geom_histogram() + scale_x_datetime(labels = date_format("%H:%M"))
