...
flux_files <- file.finder(data.path = "../../1-Raw Data/flux", start="2022-08-30", end= "2022-08-31", type="flux")
temp_files <- file.finder(data.path = "../../1-Raw Data/flux", start="2022-08-30", end= "2022-08-31", type="temperature")

data <- read.flux(flux_files, "America/New_York")

times <-read.csv("../../1-Raw Data/flux/2022-08-30/Deans_20220830_times.csv") %>% filter(is.na(Start)==FALSE, is.na(End)==FALSE, Start!="", End!="") %>% mutate(Start= mdy_hms(paste(Date, Start, sep=" "), tz="America/New_York"), End= mdy_hms(paste(Date, End, sep= " "), tz="America/New_York")) 
```
...
pdf("../../1-Raw Data/flux/flux_plots/Deans_20220830_ch4_plots.pdf", paper="a4", width=9, height=12)
marrangeGrob(grobs=compact(flux_list$ch4_plot), nrow=3, ncol=2)
dev.off()
# 
pdf("../../1-Raw Data/flux/flux_plots/Deans_20220830_co2_plots.pdf", paper="a4", width=9, height=12)
...
file_name <- "../../1-Raw Data/flux/processed-flux/Deans_20220830_Fluxes.csv"
write.csv(fluxes, file_name , row.names = F)
```

```{r}
# calculate volume added by tubing
