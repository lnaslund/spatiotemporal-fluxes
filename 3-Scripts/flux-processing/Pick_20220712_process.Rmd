...
flux_files <- file.finder(data.path = "../../1-Raw Data/flux", start="2022-07-12", end= "2022-07-13", type="flux")
temp_files <- file.finder(data.path = "../../1-Raw Data/flux", start="2022-07-12", end= "2022-07-13", type="temperature")

data <- read.flux(flux_files, "America/New_York")

times <-read.csv("../../1-Raw Data/flux/2022-07-12/Pick_20220712_times.csv") %>% filter(is.na(Diffusive.start)==FALSE, is.na(Diffusive.end)==FALSE, Diffusive.start!="", Diffusive.end!="") %>% mutate(Start= mdy_hms(paste(Date, Diffusive.start, sep=" "), tz="America/New_York"), End= mdy_hms(paste(Date, Diffusive.end, sep= " "), tz="America/New_York")) 

temp_data <- read.temp(temp_files, "America/New_York") %>% rename("date_time_sec" = "date_time")

baro <- read.csv("../../1-Raw Data/flux/2022-07-12/barometer_20220712.csv") %>% 
...
water_temp <- read.csv("../../1-Raw Data/flux/2022-07-12/pick_0.25.csv") %>%
...
pdf("../../1-Raw Data/flux/flux_plots/Pick_20220712_ch4_plots.pdf", paper="a4", width=9, height=12)
marrangeGrob(grobs=compact(flux_list$ch4_plot), nrow=3, ncol=2)
dev.off()
# 
pdf("../../1-Raw Data/flux/flux_plots/Pick_20220712_co2_plots.pdf", paper="a4", width=9, height=12)
...
# this function isn't accounting for the change in width and length or the added volume
...
fluxes <- calc_flux(flux_list, width=0.3429, length=0.3429) # just changed ppm calc above to 1e-6 needs to be run

fluxes <- fluxes %>% mutate(temporary = hm(paste(hour(start_time), minute(start_time), sep = ":"))) %>% left_join(times %>% mutate(temporary = hm(paste(hour(Start), minute(Start), sep = ":"))) %>% select(Trip, temporary),by=c("temporary")) %>% select(-temporary) %>% rename("trip"="Trip")

file_name <- "../../1-Raw Data/flux/processed-flux/Pick_20220712_Fluxes.csv"
write.csv(fluxes, file_name , row.names = F)
```

```{r}
# calculate the volume added by the tubing
# bev a line tubing
